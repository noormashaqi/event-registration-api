    ...
    public class UpdateEvent
    {
        ...
        public class Handler : IRequestHandler<Command>
        {
            ...
            public async Task Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                // التأكد من وجود الفعالية أولاً وحساب عدد المشتركين الحاليين فيها
                ...
                // منع تقليل السعة (Capacity) تحت عدد الحجوزات النشطة (قاعدة الـ 409 Conflict)
                if (request.Capacity < (int)eventInfo.ActiveRegistrations)
                    throw new InvalidOperationException($"Capacity cannot be reduced below the current number of active registrations ({eventInfo.ActiveRegistrations}).");

                // التأكد من أن التصنيف المستهدف موجود ونشط
