    ...
    public class DeleteEvent
    {
        ...
        public class Handler : IRequestHandler<Command>
        {
            ...
            public async Task Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                // منع الحذف الفعلي إذا كان هناك أي سجل تسجيل مربوط بها (قاعدة الـ 409 Conflict)
                ...
                if (registrationCount > 0)
                    throw new InvalidOperationException("An event with any registration record cannot be hard deleted.");
