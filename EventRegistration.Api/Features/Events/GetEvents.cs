    ...
    public class GetEvents
    {
        ...
        public class Handler : IRequestHandler<Query, PaginatedResult<ResultItem>>
        {
            ...
            public async Task<PaginatedResult<ResultItem>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                // جلب العدد الإجمالي بناءً على الفلاتر المحددة
                ...
                // جلب البيانات مع حساب الحجوزات النشطة والـ JOIN مع التصنيفات
                sqlBuilder.Append($@"
                    SELECT e.Id, e.Name, e.CategoryId, c.Name AS CategoryName, e.Location, e.StartAt, e.EndAt, e.Capacity, e.IsActive,
